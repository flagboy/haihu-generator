...
def create_sample_frame_data(frame_number: int, scenario: str) -> Dict[str, Any]:
    ...
    if scenario == "initial_deal":
        ...
        base_data.update({
            'player_hands': {
                '0': ['1m', '2m', '3m', '4m', '5m', '6m', '7m', '8m', '9m', '1p', '2p', '3p'],  # 1枚減
                '1': ['5p', '6p', '7p', '8p', '9p', '1s', '2s', '3s', '4s', '5s', '6s', '7s', '8s'],
                '2': ['9s', '東', '南', '西', '北', '白', '發', '中', '1m', '2m', '3m', '4m', '5m'],
                '3': ['6m', '7m', '8m', '9m', '1p', '2p', '3p', '4p', '5p', '6p', '7p', '8p', '9p']
            },
        ...
        base_data.update({
            'player_hands': {
                ...
                '1': ['5p', '6p', '7p', '8p', '9p', '1s', '2s', '3s', '4s', '5s', '6s', '7s'],  # 1枚減
                '2': ['9s', '東', '南', '西', '北', '白', '發', '中', '1m', '2m', '3m', '4m', '5m'],
                '3': ['6m', '7m', '8m', '9m', '1p', '2p', '3p', '4p', '5p', '6p', '7p', '8p', '9p']
            },
            'discarded_tiles': {
        ...
        base_data.update({
            'player_hands': {
                ...
                '2': ['9s', '東', '南', '西', '北', '白', '發', '中', '1m', '2m', '3m', '4m'],  # 1枚減
                '3': ['6m', '7m', '8m', '9m', '1p', '2p', '3p', '4p', '5p', '6p', '7p', '8p']   # 1枚減
            },
            'discarded_tiles': {
                '0': ['4p'],
                '1': ['8s'],
...
def run_tracking_demo():
    ...
    for action in result2.actions:
        logger.info(f"  行動: {action.action_type.value}, プレイヤー: {action.player.name}, 牌: {action.tile}")
    ...
    for change in changes:
        logger.info(f"  変化: {change.change_type.value}, 場所: {change.location.value}, 牌: {change.tiles}")
