...
def demo_ai_features(config_manager):
    ...
    try:
        ...
        print(f"✓ 分類テスト完了 - 予測牌: {classification.tile_name}, 信頼度: {classification.confidence:.3f}")
...
def main():
    ...
    parser.add_argument("--phase", choices=['1', '2', 'all'], default='all', help="実行するフェーズ")
    ...
    try:
        ...
        if args.phase in ['1', 'all']:
            ...
            filter_ratio = len(filtered_frames) / len(extracted_frames) * 100 if extracted_frames else 0
            ...
            if args.phase == 'all' and 'extracted_frames' in locals() and extracted_frames:
                ...
                if test_frames:
                    ...
                    for i, result in enumerate(results):
                        print(f"  フレーム{i+1}: 検出数={len(result.detections)}, "
