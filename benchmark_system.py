...
class SystemBenchmark:
    ...
    def create_test_video(self, path: str, duration_seconds: int, fps: int = 30, 
    ...
    def benchmark_video_processing(self) -> BenchmarkResult:
        ...
        try:
            ...
            self.logger.info(f"{test_name} completed: {throughput:.2f} FPS, {memory_usage:.1f}MB memory")
            return result
    ...
    def benchmark_memory_performance(self) -> BenchmarkResult:
        ...
        try:
            ...
            memory_efficiency = (peak_memory - initial_memory) / arrays_created if arrays_created > 0 else 0
            ...
            self.logger.info(f"{test_name} completed: {arrays_created} arrays, {memory_efficiency:.1f}MB/array")
            return result
    ...
    def benchmark_cpu_performance(self) -> BenchmarkResult:
        ...
        try:
            ...
            self.logger.info(f"{test_name} completed: {speedup:.2f}x speedup, {efficiency:.2%} efficiency")
            return result
    ...
    def benchmark_io_performance(self) -> BenchmarkResult:
        ...
        try:
            ...
            self.logger.info(f"{test_name} completed: Write {write_throughput:.1f}MB/s, Read {read_throughput:.1f}MB/s")
            return result
    ...
    def benchmark_system_optimization(self) -> BenchmarkResult:
        ...
        try:
            ...
            # 改善度計算
            cpu_improvement = before_metrics.cpu_usage - after_metrics.cpu_usage if hasattr(before_metrics, 'cpu_usage') and hasattr(after_metrics, 'cpu_usage') else 0
            memory_improvement = before_metrics.memory_usage - after_metrics.memory_usage if hasattr(before_metrics, 'memory_usage') and hasattr(after_metrics, 'memory_usage') else 0
            ...
            result = BenchmarkResult(
                ...
                processing_time=processing_time,
                throughput=1 / processing_time if processing_time > 0 else 0,
                memory_usage_mb=after_metrics.memory_usage if hasattr(after_metrics, 'memory_usage') else 0,
                cpu_usage_percent=after_metrics.cpu_usage if hasattr(after_metrics, 'cpu_usage') else 0,
                additional_metrics={
                    'cpu_improvement': cpu_improvement,
                    'memory_improvement': memory_improvement,
                    'optimization_type': getattr(optimization_result, 'optimization_type', 'general'),
                    'recommendations_count': len(performance_optimizer.get_optimization_recommendations())
                }
            )
            
            self.logger.info(f"{test_name} completed: CPU {cpu_improvement:+.1f}%, Memory {memory_improvement:+.1f}%")
            return result
    ...
    def _generate_benchmark_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'average_throughput': np.mean([r.throughput for r in successful_tests]) if successful_tests else 0,
            'average_memory_usage': np.mean([r.memory_usage_mb for r in successful_tests]) if successful_tests else 0,
            'average_cpu_usage': np.mean([r.cpu_usage_percent for r in successful_tests]) if successful_tests else 0,
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for result in self.results:
            if not result.success:
                ...
                recommendations.append(f"Consider optimizing {result.test_name} for better throughput")
