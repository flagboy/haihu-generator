...
class MemoryOptimizer:
    ...
    def _check_auto_optimization(self, memory_info: MemoryInfo):
        ...
        if memory_percent >= self.optimization_config['critical_threshold']:
            ...
            self.logger.info(f"Running garbage collection due to memory usage: {memory_percent:.1f}%")
            gc.collect()
    ...
    def get_memory_recommendations(self) -> List[str]:
        ...
        if latest_info.memory_percent > 90:
            ...
            recommendations.append("メモリ使用率が高いです。定期的にガベージコレクションを実行してください")
            
        # プロセスメモリに基づく推奨
        if latest_info.process_memory > 4.0:  # 4GB以上
            recommendations.append("プロセスメモリ使用量が大きいです。データの分割処理を検討してください")
        ...
        if len(self.memory_history) > 10:
            ...
            if avg_usage > 75:
                recommendations.append("平均メモリ使用率が高いです。システム設定の見直しを検討してください")
    ...
    def export_memory_report(self, output_path: str):
        ...
        try:
            ...
            report_data = {
                'current_memory_info': {
                    'total_memory_gb': self.memory_history[-1].total_memory if self.memory_history else 0,
                    'available_memory_gb': self.memory_history[-1].available_memory if self.memory_history else 0,
                    'memory_percent': self.memory_history[-1].memory_percent if self.memory_history else 0,
                    'process_memory_gb': self.memory_history[-1].process_memory if self.memory_history else 0
                },
                'memory_history': [
                    {
                        'timestamp': i,
    ...
    def estimate_memory_requirement(self, data_size_gb: float, 
                                  processing_factor: float = 2.0) -> Dict[str, Any]:
        """メモリ要件を推定"""
        try:
    ...
    def _calculate_recommended_batch_size(self, data_size_gb: float, 
