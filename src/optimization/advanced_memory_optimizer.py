...
class AdvancedMemoryOptimizer(MemoryOptimizer):
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Memory optimization completed. Freed {results['memory_freed_mb']:.2f} MB")
        ...
        while self._monitoring:
            try:
                ...
                if memory_info['percent'] > self.memory_config.get('auto_optimize_threshold', 80):
                    self.logger.warning(f"Memory usage high: {memory_info['percent']:.1f}%. Running optimization...")
                    self.optimize_memory()
                
                # メモリ使用量をログ出力（デバッグレベル）
                self.logger.debug(f"Memory usage: {memory_info['percent']:.1f}% "
