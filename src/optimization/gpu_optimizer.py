    ...
    def optimize_gpu_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if before_info and after_info:
                for before, after in zip(before_info, after_info):
                ...
                if gpus:
                    for gpu in gpus:
                        ...
                        if self.optimization_config['memory_fraction'] < 1.0:
                            memory_limit = int(8192 * self.optimization_config['memory_fraction'])  # MB
    ...
    def _check_auto_optimization(self, gpu_infos: List[GPUInfo]):
        ...
        for gpu_info in gpu_infos:
            ...
            if memory_usage_ratio >= self.optimization_config['clear_cache_threshold']:
                self.logger.warning(f"GPU {gpu_info.gpu_id} memory usage high: {memory_usage_ratio:.1%}")
                self.optimize_gpu_memory()
            
            # 温度が高い場合（警告のみ）
            if gpu_info.temperature and gpu_info.temperature > 80:
                self.logger.warning(f"GPU {gpu_info.gpu_id} temperature high: {gpu_info.temperature}°C")
    ...
    def get_gpu_recommendations(self) -> List[str]:
        ...
        for gpu_info in recent_gpus:
            ...
            if memory_usage_ratio > 0.9:
                recommendations.append(f"GPU {gpu_id}: メモリ使用率が非常に高いです（{memory_usage_ratio:.1%}）")
            elif memory_usage_ratio > 0.8:
                recommendations.append(f"GPU {gpu_id}: メモリ使用率が高いです（{memory_usage_ratio:.1%}）")
            
            # 使用率チェック
            if gpu_info.utilization > 95:
                recommendations.append(f"GPU {gpu_id}: 使用率が非常に高いです（{gpu_info.utilization}%）")
