...
class TileClassifier:
    ...
    def handle_occlusion(self, image: np.ndarray, 
                        occlusion_mask: Optional[np.ndarray] = None) -> ClassificationResult:
        """
        重なり牌の処理
        ...
        if result.confidence < self.confidence_threshold:
            ...
            # 最も信頼度の高い結果を選択
            best_result = max([result] + alternative_results, 
                            key=lambda x: x.confidence)
            return best_result
            
        return result
        
    def _preprocess_occluded_image(self, image: np.ndarray, 
