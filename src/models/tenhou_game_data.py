...
class TenhouAction:
    ...
    def __init__(self, action_type: TenhouActionType, player: int, timestamp: Optional[datetime] = None):
...
class TenhouAgariAction(TenhouAction):
    ...
    player: int
    is_tsumo: bool
    target_player: Optional[int] = None  # ロンの場合の放銃者
    han: int = 0
    fu: int = 0
    score: int = 0
    yaku: List[str] = field(default_factory=list)
    winning_tile: Optional[TenhouTile] = None
    timestamp: Optional[datetime] = None
    ...
    def to_tenhou_format(self) -> List[Any]:
        if self.is_tsumo:
            ...
            return [f"AGARI{self.player}", f"ron{self.target_player}", 
                   self.han, self.fu, self.score]
...
class TenhouPlayerState:
    ...
    def remove_tile(self, tile_notation: str) -> Optional[TenhouTile]:
        """手牌から牌を削除"""
...
class TenhouGameData:
    ...
    result: Optional[TenhouGameResult] = None
    ...
    def __post_init__(self):
        ...
        while len(self.players) < 4:
            ...
            self.players.append(TenhouPlayerState(
                player_id=player_id,
                name=f"プレイヤー{player_id + 1}"
            ))
    ...
    def get_player(self, player_id: int) -> Optional[TenhouPlayerState]:
        """プレイヤー状態を取得"""
