...
class AIPipeline:
        ...
        try:
            ...
            if detections:
                ...
                if len(tile_images) > 1:
                    ...
                    classification_results = [self.classifier.classify_tile(tile_images[0])] if tile_images else []
                
                # 検出結果と分類結果をペアリング
                for detection, classification in zip(detections, classification_results):
            ...
            # 統計更新
            self._update_stats(1, len(filtered_detections), len(filtered_classifications), processing_time)
    ...
    def process_frames_batch(self, frames: List[np.ndarray], 
                           start_frame_id: int = 0) -> List[PipelineResult]:
        """
        複数フレームをバッチ処理
    ...
    def _process_frames_parallel(self, frames: List[np.ndarray], 
                               start_frame_id: int) -> List[PipelineResult]:
        """並列フレーム処理"""
        ...
        with ThreadPoolExecutor(max_workers=self.batch_config.max_workers) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    self.logger.error(f"Parallel processing failed for frame {start_frame_id + index}: {e}")
    ...
    def _extract_tile_images(self, frame: np.ndarray, 
    ...
    def _calculate_confidence_scores(self, 
                                   classifications: List[Tuple[DetectionResult, ClassificationResult]]) -> Dict[str, float]:
        """信頼度スコアを計算"""
    ...
    def _filter_results(self, detections: List[DetectionResult],
                       classifications: List[Tuple[DetectionResult, ClassificationResult]],
                       confidence_scores: Dict[str, float]) -> Tuple[List[DetectionResult], List[Tuple[DetectionResult, ClassificationResult]]]:
        """結果をフィルタリング"""
        ...
        filtered_classifications = [
            (det, cls) for det, cls in classifications
    ...
    def _update_stats(self, frames: int, detections: int, 
                     classifications: int, processing_time: float):
        """統計情報を更新"""
