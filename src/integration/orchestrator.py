...
class VideoProcessingOrchestrator:
        ...
        try:
            ...
            if not extraction_result['success']:
                return ProcessingResult(
                    ...
                    errors=[f"フレーム抽出に失敗: {extraction_result.get('error', 'Unknown error')}"]
                )
    ...
    def _process_frames_with_ai(self,
                               frames: List[Any],
                               options: ProcessingOptions) -> List[Dict[str, Any]]:
        """
        フレームをAIで処理
        ...
        Returns:
            AI処理結果のリスト
        """
        batch_size = options.batch_size or self.system_config.get('constants', {}).get('default_batch_size', 32)
        results = []
