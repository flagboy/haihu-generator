...
class StatisticsCollector:
    ...
    def collect_statistics(self,
                          ...
                          game_results: Optional[Any] = None) -> Dict[str, Any]:
        """
        処理結果から統計情報を収集
    ...
    def _collect_ai_statistics(self, ai_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'detection_rate': sum(1 for c in detection_counts if c > 0) / len(ai_results) if ai_results else 0
        }
        ...
        for tile_type, data in tile_types.items():
            tile_statistics[tile_type] = {
                ...
                'frequency': data['count'] / stats['total_detections'] if stats['total_detections'] > 0 else 0
            }
    ...
    def _collect_game_statistics(self, game_results: Any) -> Dict[str, Any]:
        ...
        if hasattr(game_results, 'rounds'):
            ...
            stats['completed_rounds'] = sum(1 for r in game_results.rounds if r.get('completed', False))
    ...
    def _collect_performance_statistics(self, processing_result: Any) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'frames_per_second': processing_result.frame_count / processing_result.processing_time if processing_result.processing_time > 0 else 0,
            'tiles_per_second': processing_result.detected_tiles / processing_result.processing_time if processing_result.processing_time > 0 else 0
        }
    ...
    def format_statistics_report(self, statistics: Dict[str, Any]) -> str:
        ...
        if 'ai_statistics' in statistics:
            ...
            lines.append(f"  フレームあたり平均検出数: {ai_stats.get('average_detections_per_frame', 0):.2f}")
            ...
            if 'confidence_statistics' in ai_stats:
                ...
                lines.append(f"  信頼度範囲: {conf_stats.get('min', 0):.3f} - {conf_stats.get('max', 0):.3f}")
