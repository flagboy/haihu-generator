...
from .table import GameType, Table, TableState
from .turn import Action, ActionType, Turn
...
class GameState:
    ...
    def __init__(self, game_type: GameType = GameType.HANCHAN):
        ...
        # 基本コンポーネント
        self.players = {pos: Player(pos, f"Player{pos.value + 1}") for pos in PlayerPosition}
        self.table = Table(game_type)
        self.turn_manager = Turn()

        # ゲーム状態
        self.phase = GamePhase.WAITING
        self.current_frame: FrameState | None = None
    ...
    def update_from_frame_detection(self, frame_data: dict[str, Any]) -> bool:
        ...
        # 新しいフレーム状態を作成
        frame_state = FrameState(frame_number=frame_number, timestamp=timestamp)
    ...
    def _process_detection_results(
        self, frame_data: dict[str, Any], frame_state: FrameState
    ) -> bool:
        """
        検出結果を処理
