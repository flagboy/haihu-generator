...
from ..game.turn import Action, ActionType, TurnState
...
class GameRecord:
    ...
    def __post_init__(self):
        ...
        if not self.final_scores:
            self.final_scores = {pos: 25000 for pos in PlayerPosition}
...
class RoundRecord:
    ...
    def __post_init__(self):
        ...
        if not self.actions:
            self.actions = []
        if not self.scores:
            self.scores = {pos: 25000 for pos in PlayerPosition}
...
class HistoryManager:
    ...
    def start_new_round(self, round_number: int, round_name: str, 
                       dealer: PlayerPosition) -> RoundRecord:
        """
        新しい局を開始
    ...
    def complete_current_round(self, result: Optional[Dict[str, Any]] = None,
                             scores: Optional[Dict[PlayerPosition, int]] = None):
        """
        現在の局を完了
    ...
    def complete_current_game(self, final_scores: Optional[Dict[PlayerPosition, int]] = None):
        """
        現在のゲームを完了
    ...
    def export_to_tenhou_format(self, game_record: Optional[GameRecord] = None) -> str:
        """
        天鳳形式で牌譜をエクスポート
    ...
    def export_to_tenhou_json_format(self, game_record: Optional[GameRecord] = None) -> Dict[str, Any]:
        """
    ...
    def _format_players_for_tenhou(self, players: Dict[PlayerPosition, str]) -> List[Dict[str, Any]]:
    ...
    def _convert_round_to_tenhou_json(self, round_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tenhou_round = {
            ...
            'final_scores': list(round_data['scores'].values()) if round_data.get('scores') else [25000, 25000, 25000, 25000]
        }
    ...
    def _convert_action_to_tenhou_json(self, action_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """行動を天鳳JSON形式に変換"""
    ...
    def search_actions(self, action_type: Optional[ActionType] = None,
                      player: Optional[PlayerPosition] = None,
                      tile: Optional[str] = None) -> List[Action]:
        """
        条件に基づいて行動を検索
    ...
    def __repr__(self) -> str:
        """詳細な文字列表現"""
        return (f"HistoryManager(games={self.total_games}, rounds={self.total_rounds}, "
