...
class TileMovement:
    ...
    to_player: Optional[PlayerPosition] = None
...
class ChangeEvent:
    ...
    player: Optional[PlayerPosition]
    tiles: List[str]
...
class ChangeAnalyzer:
    ...
    def analyze_frame_changes(self, previous_frame: Dict[str, Any],
                            current_frame: Dict[str, Any],
                            frame_number: int) -> List[ChangeEvent]:
        """
        フレーム間の変化を分析
        ...
        # 各プレイヤーの手牌変化を分析
        change_events.extend(self._analyze_hand_changes(
            previous_frame, current_frame, frame_number
        ))
        
        # 捨て牌の変化を分析
        change_events.extend(self._analyze_discard_changes(
            previous_frame, current_frame, frame_number
        ))
        
        # 鳴きの変化を分析
        change_events.extend(self._analyze_call_changes(
            previous_frame, current_frame, frame_number
        ))
    ...
    def _analyze_hand_changes(self, previous_frame: Dict[str, Any],
                            current_frame: Dict[str, Any],
                            frame_number: int) -> List[ChangeEvent]:
        """手牌の変化を分析"""
        changes = []
    ...
    def _analyze_discard_changes(self, previous_frame: Dict[str, Any],
                               current_frame: Dict[str, Any],
                               frame_number: int) -> List[ChangeEvent]:
        """捨て牌の変化を分析"""
        changes = []
    ...
    def _analyze_call_changes(self, previous_frame: Dict[str, Any],
                            current_frame: Dict[str, Any],
                            frame_number: int) -> List[ChangeEvent]:
        """鳴きの変化を分析"""
        changes = []
    ...
    def _detect_tile_list_changes(self, prev_tiles: List[str], curr_tiles: List[str],
                                location: ChangeLocation, player: PlayerPosition,
                                frame_number: int) -> Optional[ChangeEvent]:
        """牌リストの変化を検出"""
    ...
    def _calculate_change_confidence(self, prev_tiles: List[str], curr_tiles: List[str],
    ...
    def _infer_tile_movements(self, change_events: List[ChangeEvent],
                            frame_number: int) -> List[TileMovement]:
        """変化イベントから牌の移動を推定"""
        movements = []
    ...
    def _is_related_movement(self, event: ChangeEvent, movement: TileMovement) -> bool:
        ...
        # 同じフレームで同じ牌が関わっている場合
        return (movement.frame_number == event.frame_number and
                movement.tile in event.tiles)
    ...
    def __repr__(self) -> str:
        """詳細な文字列表現"""
        return (f"ChangeAnalyzer(changes={self.total_changes_analyzed}, "
