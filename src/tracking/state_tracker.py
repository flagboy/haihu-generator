...
from ..game.game_state import GameState, FrameState, GamePhase
...
class StateTracker:
    ...
    def _check_inconsistencies(self, frame_data: Dict[str, Any],
                             detection_result: DetectionResult) -> List[InconsistencyReport]:
        """
        矛盾をチェック
    ...
    def _check_hand_size_inconsistencies(self, frame_data: Dict[str, Any],
                                       frame_number: int) -> List[InconsistencyReport]:
        """手牌枚数の矛盾をチェック"""
        inconsistencies = []
    ...
    def _check_tile_count_inconsistencies(self, frame_data: Dict[str, Any],
                                        frame_number: int) -> List[InconsistencyReport]:
        """牌の総数の矛盾をチェック"""
        inconsistencies = []
    ...
    def _check_action_inconsistencies(self, detection_result: DetectionResult,
                                    frame_number: int) -> List[InconsistencyReport]:
        """行動の論理的矛盾をチェック"""
        inconsistencies = []
    ...
    def _handle_inconsistencies(self, inconsistencies: List[InconsistencyReport],
                              frame_number: int):
        """矛盾を処理"""
    ...
    def _create_snapshot(self, frame_number: int, timestamp: float,
    ...
    def get_tracking_statistics(self) -> Dict[str, Any]:
        """追跡統計を取得"""
        success_rate = (self.successful_updates / self.total_frames_processed 
    ...
    def get_recent_inconsistencies(self, count: int = 10) -> List[InconsistencyReport]:
        """最近の矛盾を取得"""
        return self.inconsistencies[-count:] if len(self.inconsistencies) > count else self.inconsistencies
    ...
    def __repr__(self) -> str:
        """詳細な文字列表現"""
        return (f"StateTracker(state={self.tracking_state.value}, "
