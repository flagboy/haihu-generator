...
class QualityValidator:
    ...
    def validate_record_file(self, record_path: str, 
    ...
    def validate_record_data(self, record_data: Dict[str, Any],
                           validation_level: ValidationLevel = None) -> ValidationResult:
        """
        牌譜データを検証
        ...
        try:
            ...
            self.logger.info(f"Validation completed. Score: {overall_score:.1f}, Success: {success}")
            return result
    ...
    def _load_record_file(self, record_path: str) -> Optional[Dict[str, Any]]:
        """牌譜ファイルを読み込み"""
        try:
    ...
    def _validate_structure(self, record_data: Dict[str, Any], 
                          validation_level: ValidationLevel) -> Tuple[float, List[ValidationIssue], Dict[str, Any]]:
        """構造検証"""
        issues = []
        statistics = {}
        score = 100.0
    ...
    def _validate_content(self, record_data: Dict[str, Any],
                        validation_level: ValidationLevel) -> Tuple[float, List[ValidationIssue], Dict[str, Any]]:
        """内容検証"""
        issues = []
        statistics = {}
    ...
    def _validate_logic(self, record_data: Dict[str, Any],
                       validation_level: ValidationLevel) -> Tuple[float, List[ValidationIssue], Dict[str, Any]]:
        """論理検証"""
        issues = []
        statistics = {}
        ...
        try:
            ...
            issues.append(ValidationIssue(
                ...
                "error",
                f"Logic validation error: {str(e)}"
            ))
            score = 0
        
        return score, issues, statistics
    
    def _validate_consistency(self, record_data: Dict[str, Any],
                            validation_level: ValidationLevel) -> Tuple[float, List[ValidationIssue], Dict[str, Any]]:
        """一貫性検証"""
        issues = []
        statistics = {}
    ...
    def _validate_tiles(self, record_data: Dict[str, Any]) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
        """牌の妥当性を検証"""
        issues = []
    ...
    def _validate_round_logic(self, round_data: Dict[str, Any], round_index: int) -> List[ValidationIssue]:
        """ラウンドの論理を検証"""
        issues = []
        ...
        if 'actions' not in round_data:
            issues.append(ValidationIssue(
                ValidationCategory.LOGIC,
                "warning",
                f"Round {round_index} has no actions"
            ))
    ...
    def _check_tile_conservation(self, record_data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        if total_tiles < limits['min_total_tiles']:
            issues.append(ValidationIssue(
                ValidationCategory.LOGIC,
                "warning",
                f"Too few tiles detected: {total_tiles}"
            ))
        elif total_tiles > limits['max_total_tiles']:
            issues.append(ValidationIssue(
                ValidationCategory.LOGIC,
                "warning",
                f"Too many tiles detected: {total_tiles}"
            ))
