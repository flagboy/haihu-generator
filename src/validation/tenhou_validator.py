...
from ..models.tenhou_game_data import TenhouGameData, TenhouAction, TenhouActionType
...
class TenhouValidator:
    ...
    TENHOU_ACTION_PATTERN = re.compile(r'^[TD]\d+|N\d+|REACH\d+|AGARI\d+|RYUU$')
    ...
    def _validate_single_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """単一アクションの検証"""
    ...
    def _validate_draw_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """ツモアクションの検証"""
        if len(action) < 2:
    ...
    def _validate_discard_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """打牌アクションの検証"""
        if len(action) < 2:
    ...
    def _validate_call_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """鳴きアクションの検証"""
        if len(action) < 3:
    ...
    def _validate_riichi_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """リーチアクションの検証"""
        if len(action) < 2:
    ...
    def _validate_agari_action(self, action: List[Any], index: int, result: ValidationResult) -> None:
        """和了アクションの検証"""
        if len(action) < 5:
    ...
    def _validate_log_score_consistency(self, log: List[List[Any]], scores: List[int], result: ValidationResult) -> None:
        """ログとスコアの整合性検証"""
        # 簡易的な整合性チェック
        ...
        if abs(total_score - expected_total) > 1000:  # 1000点の誤差を許容
            result.add_warning(f"総得点が期待値と大きく異なります (実際: {total_score}, 期待: {expected_total})")
    
    def _validate_action_sequence(self, actions: List[TenhouAction], result: ValidationResult) -> None:
        """アクションシーケンスの検証"""
        if not actions:
    ...
    def _validate_game_rule_consistency(self, game_data: TenhouGameData, result: ValidationResult) -> None:
        """ゲームルール整合性の検証"""
        # 赤ドラ設定と実際の牌使用の整合性
