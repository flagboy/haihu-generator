...
class ConfidenceCalculator:
    ...
    def calculate_overall_confidence(self, detection_results: List[Any],
                                   classification_results: List[Any],
                                   game_state_data: Optional[Dict[str, Any]] = None) -> ConfidenceScore:
        """
        総合信頼度を計算
        ...
        try:
            ...
            temporal_score = self._calculate_temporal_confidence(
                detection_results, game_state_data
            )
    ...
    def _calculate_classification_confidence(self, classification_results: List[Any]) -> float:
        ...
        try:
            ...
            if mean_confidence + median_confidence > 0:
                harmonic_mean = 2 * mean_confidence * median_confidence / (mean_confidence + median_confidence)
            else:
                harmonic_mean = 0.0
    ...
    def _calculate_consistency_confidence(self, detection_results: List[Any],
                                        classification_results: List[Any]) -> float:
        """一貫性信頼度を計算"""
        try:
            ...
            classification_std = np.std(classification_counts) if len(classification_counts) > 1 else 0
    ...
    def _calculate_temporal_confidence(self, detection_results: List[Any],
                                     game_state_data: Optional[Dict[str, Any]]) -> float:
        """時系列信頼度を計算"""
        try:
            if not detection_results:
                return 0.0
    ...
    def _generate_confidence_recommendations(self, detection_score: float,
                                           classification_score: float,
                                           consistency_score: float,
        ...
        if detection_score < 0.6:
            recommendations.append("検出精度が低いです。検出モデルの調整や閾値の見直しを検討してください")
        
        # 分類スコアに基づく推奨
        if classification_score < 0.6:
            recommendations.append("分類精度が低いです。分類モデルの再訓練や前処理の改善を検討してください")
        
        # 一貫性スコアに基づく推奨
        if consistency_score < 0.6:
            recommendations.append("データの一貫性が低いです。入力データの品質確認や処理パラメータの調整を検討してください")
        
        # 時系列スコアに基づく推奨
        if temporal_score < 0.6:
            recommendations.append("時系列の安定性が低いです。フレーム抽出間隔の調整や平滑化処理を検討してください")
        ...
        if avg_score >= 0.8:
            ...
            recommendations.append("良好な信頼度です。さらなる改善のため細かな調整を検討してください")
        else:
            recommendations.append("信頼度が低いです。システム全体の見直しが必要です")
        
        return recommendations if recommendations else ["信頼度評価が完了しました"]
    
    def calculate_frame_confidence(self, detection_result: Any,
                                 classification_result: Any) -> float:
        """単一フレームの信頼度を計算"""
        try:
            detection_conf = 0.0
            ...
            if hasattr(detection_result, 'detections'):
                if detection_result.detections:
                    detection_confidences = [d.confidence for d in detection_result.detections if hasattr(d, 'confidence')]
            ...
            if hasattr(classification_result, 'classifications'):
                if classification_result.classifications:
                    classification_confidences = [c[1].confidence for c in classification_result.classifications if hasattr(c[1], 'confidence')]
                    classification_conf = np.mean(classification_confidences) if classification_confidences else 0.0
            
            # フレーム信頼度（検出と分類の調和平均）
            if detection_conf + classification_conf > 0:
                frame_confidence = 2 * detection_conf * classification_conf / (detection_conf + classification_conf)
            else:
    ...
    def export_confidence_report(self, confidence_score: ConfidenceScore, 
                               output_path: str):
        """信頼度レポートをエクスポート"""
        try:
            import json
