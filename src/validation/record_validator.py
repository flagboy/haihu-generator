...
class RecordValidator:
    ...
    def _setup_validation_rules(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tile_rules": {
                ...
                "hand_size_limits": {"min": 13, "max": 14},
                "discard_limits": {"min": 0, "max": 25},
            },
        }
    ...
    def _validate_tenhou_init(
        self, init_element: str, init_index: int, rules: dict[str, Any]
    ...
    def _extract_tiles_from_tenhou(self, record_data: str) -> list[str]:
        ...
        # 数値から牌名への変換が必要
        tile_patterns = [r"<T(\d+)/>", r"<D(\d+)/>", r"<U(\d+)/>"]
    ...
    def _convert_tenhou_tile_id(self, tile_id: int) -> str | None:
        """天鳳の牌IDを牌名に変換"""
        try:
            # 天鳳の牌ID変換ルール（簡易版）
            if 0 <= tile_id <= 35:
