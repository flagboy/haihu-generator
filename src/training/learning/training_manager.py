...
class TrainingManager(LoggerMixin):
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        # 学習関連ディレクトリ設定
        self.training_root = Path(self.config.get("training", {}).get("training_root", "data/training"))
    ...
    def _prepare_dataset(self, dataset_version_id: str) -> Optional[AnnotationData]:
        """データセットを準備"""
        try:
            # バージョン情報を取得
    ...
    def _split_dataset(self, annotation_data: AnnotationData, val_split: float, 
                      test_split: float, seed: int) -> Tuple[AnnotationData, AnnotationData, AnnotationData]:
        """データセットを訓練/検証/テストに分割"""
        import random
        random.seed(seed)
        ...
        self.logger.info(f"データ分割完了: 訓練={len(train_frames)}, 検証={len(val_frames)}, テスト={len(test_frames)}")
        
        return train_data, val_data, test_data
    
    def _create_annotation_data_from_frames(self, original_data: AnnotationData, 
                                          frame_list: List[Tuple[str, Any]]) -> AnnotationData:
    ...
    def _prepare_model(self, config: TrainingConfig) -> nn.Module:
        ...
        if config.transfer_learning and config.pretrained_model_path:
            ...
            if os.path.exists(config.pretrained_model_path):
                ...
                self.logger.warning(f"事前学習済みモデルが見つかりません: {config.pretrained_model_path}")
    ...
    def compare_models(self, session_ids: List[str]) -> Dict[str, Any]:
        ...
        for session_id in session_ids:
            ...
            if session and session.final_metrics:
                ...
                comparison_data["sessions"].append({
                    ...
                    "training_time": (session.end_time - session.start_time).total_seconds() if session.end_time else None
                })
        ...
        for metric in all_metrics:
            ...
            for session in valid_sessions:
                if metric in session.final_metrics:
                    values.append({
                        "session_id": session.session_id,
                        "value": session.final_metrics[metric]
                    })
        ...
        if "val_accuracy" in comparison_data["metrics_comparison"]:
            best_session_id = comparison_data["metrics_comparison"]["val_accuracy"]["best"]["session_id"]
