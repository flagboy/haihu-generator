...
class ModelEvaluator(LoggerMixin):
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        # 評価結果保存ディレクトリ
        self.evaluation_root = Path(self.config.get("training", {}).get("evaluation_root", "data/training/evaluation"))
    ...
    def evaluate_model(self, model_path: str, test_data: AnnotationData, 
                      model_type: str, save_results: bool = True) -> Dict[str, float]:
        """
        モデルを評価
    ...
    def _load_model(self, model_path: str, model_type: str) -> Optional[nn.Module]:
        """モデルを読み込み"""
        try:
    ...
    def _evaluate_detection_model(self, model: nn.Module, test_loader, 
                                 test_dataset: TileDataset) -> Dict[str, float]:
        """検出モデルを評価"""
        model.eval()
        ...
        metrics['map_50'] = self._calculate_map(
            all_bbox_preds, all_bbox_targets, all_conf_preds, all_class_preds, all_class_targets, iou_threshold=0.5
        )
        
        return metrics
    
    def _evaluate_classification_model(self, model: nn.Module, test_loader,
                                     test_dataset: TileDataset) -> Dict[str, float]:
        """分類モデルを評価"""
        model.eval()
        ...
        if all_probs.shape[1] > 1:
            top_k_acc = self._calculate_top_k_accuracy(all_probs, all_targets, k=min(5, all_probs.shape[1]))
    ...
    def _calculate_map(self, bbox_preds: List, bbox_targets: List, conf_preds: List,
                      class_preds: List, class_targets: List, iou_threshold: float = 0.5) -> float:
    ...
    def create_confusion_matrix(self, model_path: str, test_data: AnnotationData,
    ...
    def create_detection_visualization(self, model_path: str, test_data: AnnotationData,
        ...
        try:
            ...
            with torch.no_grad():
                for i, frame in enumerate(samples[:min(5, len(samples))]):
                    ...
                    for tile in frame.tiles:
                        bbox = tile.bbox
                        rect = plt.Rectangle((bbox.x1, bbox.y1), bbox.width, bbox.height,
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            model_name = Path(model_path).stem
            output_path = self.visualizations_dir / f"{model_name}_{timestamp}_detection_results.png"
    ...
    def generate_evaluation_report(self, model_path: str, test_data: AnnotationData,
                                 model_type: str, training_history: Optional[List[Dict[str, Any]]] = None) -> str:
        """
        総合評価レポートを生成
        ...
        try:
            ...
            html_content = self._generate_html_report(
                model_path, model_type, metrics, confusion_matrix_path,
                learning_curves_path, detection_viz_path, training_history
            )
    ...
    def _generate_html_report(self, model_path: str, model_type: str, metrics: Dict[str, float],
                            confusion_matrix_path: str, learning_curves_path: str,
                            detection_viz_path: str, training_history: Optional[List[Dict[str, Any]]]) -> str:
        """HTMLレポートを生成"""
    ...
    def compare_models(self, model_paths: List[str], test_data: AnnotationData,
