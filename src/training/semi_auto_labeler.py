...
from .annotation_data import AnnotationData, FrameAnnotation, TileAnnotation, BoundingBox
from ..detection.tile_detector import TileDetector, DetectionResult
...
class PredictionResult:
    ...
    def __init__(self, frame_annotation: FrameAnnotation, 
    ...
    def _calculate_confidence_scores(self) -> Dict[str, float]:
        ...
        return {
            ...
            "detection": np.mean([det.confidence for det in self.detection_results]) if self.detection_results else 0.0,
            "classification": np.mean([cls.confidence for cls in self.classification_results]) if self.classification_results else 0.0,
...
class VisualizationGenerator:
    ...
    def generate_prediction_visualization(self, image_path: str, 
                                        prediction_result: PredictionResult) -> np.ndarray:
        """
        予測結果の可視化画像を生成
    ...
    def _draw_tile_annotation(self, image: np.ndarray, tile: TileAnnotation):
        ...
        cv2.rectangle(image, (bbox.x1, bbox.y1 - text_height - 5), 
                     (bbox.x1 + text_width, bbox.y1), color, -1)
        
        # テキスト描画
        cv2.putText(image, label, (bbox.x1, bbox.y1 - 5), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # 遮蔽情報
        if tile.is_occluded:
            cv2.putText(image, "OCCLUDED", (bbox.x1, bbox.y2 + 15), 
    ...
    def _draw_statistics(self, image: np.ndarray, confidence_scores: Dict[str, float]):
        ...
        for i, text in enumerate(stats_text):
            cv2.putText(image, text, (15, 30 + i * 20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    def generate_comparison_view(self, image_path: str, 
                               prediction_result: PredictionResult,
                               ground_truth: Optional[FrameAnnotation] = None) -> np.ndarray:
        """
        予測結果と正解の比較表示を生成
        ...
        for tile in ground_truth.tiles:
            ...
            cv2.putText(gt_vis, tile.tile_id, (bbox.x1, bbox.y1 - 5), 
        ...
        # ラベル
        cv2.putText(comparison_image, "Prediction", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(comparison_image, "Ground Truth", (w + 10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
class SemiAutoLabeler(LoggerMixin):
    ...
    def predict_frame_annotations(self, frame_annotation: FrameAnnotation) -> PredictionResult:
        ...
        if self.auto_area_classification:
            area_classified_detections = self.tile_detector.classify_tile_areas(image, detection_results)
        else:
            ...
            for detection in detections:
                ...
                if tile_image.size > 0:
                    ...
                    if classification_result.confidence >= self.confidence_threshold:
                        # 遮蔽検出
                        is_occluded, occlusion_ratio = self._detect_occlusion(tile_image) if self.enable_occlusion_detection else (False, 0.0)
        ...
        prediction_result = PredictionResult(updated_frame, detection_results, classification_results)
        
        self.logger.info(f"フレーム予測完了: {len(predicted_tiles)}個の牌を検出")
        return prediction_result
    
    def batch_predict_annotations(self, frame_annotations: List[FrameAnnotation]) -> List[PredictionResult]:
        """
        複数フレームの一括予測
    ...
    def save_predictions(self, prediction_results: List[PredictionResult], 
                        output_name: str = "predictions") -> bool:
        """
        予測結果を保存
    ...
    def generate_visualizations(self, prediction_results: List[PredictionResult], 
                              output_name: str = "visualizations") -> bool:
        """
        予測結果の可視化を生成
    ...
    def _calculate_prediction_statistics(self, prediction_results: List[PredictionResult]) -> Dict[str, Any]:
        """予測統計を計算"""
    ...
    def get_labeling_statistics(self) -> Dict[str, Any]:
        ...
        if self.predictions_dir.exists():
            ...
            stats["prediction_files"] = len([f for f in prediction_files if not f.name.endswith("_stats.json")])
