...
class FrameExtractor(LoggerMixin):
    ...
    def extract_training_frames(self, video_path: str, 
                              annotation_data: Optional[AnnotationData] = None) -> List[FrameAnnotation]:
        """
        教師データ作成用フレームを抽出
    ...
    def _extract_frames_with_quality_filter(self, video_path: str, output_dir: str, 
        ...
        try:
            ...
            while True:
                ...
                if frame_count % 1000 == 0:
                    self.logger.info(f"フレーム処理進捗: {frame_count}フレーム処理, {extracted_count}フレーム抽出")
    ...
    def _apply_diversity_filter(self, frames: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for frame in frames[1:]:
            ...
            for selected_frame in diverse_frames[-5:]:  # 直近5フレームと比較
                ...
                if self._calculate_frame_similarity(current_image, selected_image) > (1.0 - self.diversity_threshold):
                    is_diverse = False
                    break
    ...
    def _select_best_frames(self, frames: List[Dict[str, Any]], max_count: int) -> List[Dict[str, Any]]:
    ...
    def extract_frames_around_timestamp(self, video_path: str, timestamp: float, 
                                      window_seconds: float = 5.0, 
                                      max_frames: int = 10) -> List[FrameAnnotation]:
        """
        指定時刻周辺のフレームを抽出
    ...
    def _is_scene_suitable_for_training(self, video_path: str, start_time: float, 
                                      end_time: float) -> bool:
