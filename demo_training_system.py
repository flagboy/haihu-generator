...
def create_sample_video(output_path: str, duration: int = 10, fps: int = 30):
    ...
    for frame_num in range(total_frames):
        ...
        cv2.rectangle(
            frame, (hand_area[0], hand_area[1]), (hand_area[2], hand_area[3]), (80, 120, 80), -1
        )
        ...
        for i in range(13):  # 手牌13枚
            ...
            tile_char = str((i % 9) + 1)
            cv2.putText(
                frame, tile_char, (x + 12, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2
            )
...
def demo_annotation_data():
    ...
    # 動画アノテーションを作成
    video_info = {"duration": 10.0, "fps": 30.0, "width": 640, "height": 480}
...
def demo_dataset_manager():
    ...
    version_id = dataset_manager.create_dataset_version(
        annotation_data, "v1.0.0", "初回デモバージョン"
    )
    if version_id:
...
def main():
    ...
    try:
        ...
        # クリーンアップ
        cleanup_files = ["demo_video.mp4", "demo_annotations.json"]
