...
""")
    ...
    def create_test_video(self, path: str, duration_seconds: int, fps: int = 30, resolution: tuple = (1920, 1080)):
        """テスト用動画を作成"""
    ...
    def test_single_video_processing_performance(self, performance_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # モックコンポーネントでテスト
            with patch('src.video.video_processor.VideoProcessor') as mock_video_processor_class, \
                ...
                mock_video_processor.extract_frames.return_value = {
                    'success': True,
                    'frames': [np.zeros((720, 1280, 3), dtype=np.uint8) for _ in range(60)]  # 30秒 * 2fps
                }
                ...
                def simulate_ai_processing(frames):
                    ...
                    return [
                        Mock(
                            frame_id=i,
                            detections=[Mock(bbox=[10, 10, 50, 50], confidence=0.8) for _ in range(5)],
...
""")
        ...
        try:
            ...
            # システム統合でワーカー数制限をテスト
            with patch('src.video.video_processor.VideoProcessor'), \
...
""")
        ...
        try:
            ...
            for result in performance_results:
                print(f"Load {result['load_level']}: {result['processing_time']:.3f}s, "
