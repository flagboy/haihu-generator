...
from src.pipeline.ai_pipeline import AIPipeline, BatchProcessingConfig, PipelineResult
...
class TestAIPipeline:
    ...
    def config_manager(self):
        ...
        config = {
            "ai": {
                "detection": {"model_type": "cnn", "confidence_threshold": 0.5},
                "classification": {"model_type": "cnn", "num_classes": 37},
                "training": {"batch_size": 8},
            },
            "system": {"max_workers": 4, "gpu_enabled": False},
        }
    ...
    def pipeline(self, config_manager):
        """AIPipelineのフィクスチャ"""
        with (
            patch("src.pipeline.ai_pipeline.TileDetector"),
    ...
    def mock_classifications(self):
        ...
        return [
            ClassificationResult(
                tile_name="1m", confidence=0.9, class_id=0, probabilities={"1m": 0.9}
            ),
            ClassificationResult(
                tile_name="2p", confidence=0.8, class_id=1, probabilities={"2p": 0.8}
            ),
        ]
    ...
    def test_extract_tile_images_invalid_bbox(self, pipeline, sample_frame):
        ...
        invalid_detections = [
            ...
            DetectionResult(
                bbox=(1000, 1000, 1050, 1050), confidence=0.7, class_id=0, class_name="tile"
            ),
        ]
    ...
    def test_calculate_confidence_scores(self, pipeline, mock_detections, mock_classifications):
        """信頼度スコア計算テスト"""
        classifications = list(zip(mock_detections, mock_classifications, strict=False))
...
class TestBatchProcessingConfig:
    ...
    def test_batch_config_creation(self):
        ...
        config = BatchProcessingConfig(
            batch_size=16, max_workers=8, enable_parallel=False, confidence_filter=0.7
        )
