...
from src.game.game_state import FrameState, GamePhase, GameState
...
class TestGameState:
    ...
    def test_update_from_frame_detection(self):
        ...
        frame_data = {
            "frame_number": 1,
            "timestamp": time.time(),
            "player_hands": {"0": ["1m", "2m", "3m"], "1": ["4p", "5p", "6p"]},
            "discarded_tiles": {"0": ["7s"], "1": []},
        }
    ...
    def test_action_inference(self):
        """行動推定のテスト"""
        # 捨て牌が増加するフレーム
        frame1_data = {"frame_number": 1, "discarded_tiles": {"0": []}}
        self.game_state.update_from_frame_detection(frame1_data)
...
class TestFrameState:
    ...
    def test_frame_state_creation(self):
        """フレーム状態作成のテスト"""
        frame_state = FrameState(frame_number=1, timestamp=time.time())
