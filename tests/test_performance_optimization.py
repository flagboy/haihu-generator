...
from src.models.tenhou_game_data import TenhouGameData, TenhouPlayerState, TenhouGameRule
from src.models.tenhou_game_data import TenhouDrawAction, TenhouDiscardAction, TenhouTile
...
class TestPerformanceOptimization:
    ...
    def _create_large_game_data(self) -> TenhouGameData:
        ...
        for i in range(1000):
            if i % 2 == 0:
                action = TenhouDrawAction(
                    player=i % 4,
                    tile=TenhouTile(f"{(i % 9) + 1}m")
                )
            else:
                action = TenhouDiscardAction(
                    player=i % 4,
                    tile=TenhouTile(f"{(i % 9) + 1}p"),
                    is_riichi=False
                )
    ...
    def test_json_conversion_speed(self):
        ...
        assert actual_time < target_time, f"変換時間が目標を超過: {actual_time:.3f}s > {target_time:.3f}s"
    ...
    def test_memory_usage_optimization(self):
        ...
        assert memory_increase < max_memory_increase, f"メモリ使用量増加が過大: {memory_increase:.1f}MB"
    ...
    def test_cache_size_limit(self):
        ...
        try:
            ...
            assert len(self.formatter._tile_cache) <= 10, f"キャッシュサイズが制限を超過: {len(self.formatter._tile_cache)}"
