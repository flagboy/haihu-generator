...
from src.models.tenhou_game_data import (
    TenhouGameData, TenhouPlayerState, TenhouGameRule, TenhouGameResult,
    TenhouDrawAction, TenhouDiscardAction, TenhouCallAction, TenhouRiichiAction,
    TenhouAgariAction, TenhouTile, TenhouCallType, TenhouGameType
)
...
class TestTenhouValidator:
    ...
    def test_is_valid_tenhou_tile(self):
        ...
        for tile in invalid_tiles:
            assert not self.validator._is_valid_tenhou_tile(tile), f"牌 '{tile}' が有効と判定されました"
    ...
    def test_error_handling_in_validation(self, mock_logger):
        ...
        with patch.object(self.validator, '_validate_basic_structure', side_effect=Exception("テスト例外")):
            result = self.validator.validate_tenhou_json(self.valid_tenhou_json)
    ...
    def test_regex_patterns(self):
        ...
        for action in invalid_actions:
            assert not action_pattern.match(action), f"アクションパターンが '{action}' に誤ってマッチしました"