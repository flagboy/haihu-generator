...
class TestVideoProcessingOrchestrator:
    ...
    def test_process_video_success(self, orchestrator):
        """正常な動画処理テスト"""
        options = ProcessingOptions(enable_optimization=True, enable_validation=True, batch_size=5)
    ...
    def test_process_frames_with_ai(self, orchestrator):
        ...
        # バッチ処理が適切に呼び出されたか確認
        assert (
            orchestrator.ai_pipeline.process_frames_batch.call_count == 4
        )  # 20フレーム / 5バッチサイズ
    ...
    def test_convert_ai_to_game_data(self, orchestrator):
        ...
        assert game_data["tiles"][0]["area"] in [
            "player_top",
            "player_bottom",
            "player_left",
            "player_right",
    ...
    def test_collect_statistics(self, orchestrator):
        ...
        ai_results = [
            {
                "frame_id": i,
                "classifications": [(Mock(), Mock(confidence=0.8 + i * 0.02)) for _ in range(2)],
            }
            for i in range(5)
        ]

        game_results = Mock()
        game_results.get_statistics.return_value = {"rounds": 2, "actions": 30}
