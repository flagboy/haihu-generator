...
class TestTenhouActions:
    ...
    def test_call_action(self):
        ...
        action = TenhouCallAction(
            player=3, call_type=TenhouCallType.PON, tiles=tiles, from_player=0
        )
    ...
    def test_agari_action(self):
        """和了アクションのテスト"""
        # ツモ和了
        tsumo_action = TenhouAgariAction(player=1, is_tsumo=True, han=4, fu=30, score=7700)
        ...
        ron_action = TenhouAgariAction(
            player=2, is_tsumo=False, target_player=3, han=2, fu=40, score=2600
        )
...
class TestTenhouGameRule:
    ...
    def test_to_tenhou_format(self):
        ...
        expected = {"disp": "テストルール", "aka": 1, "kuitan": 0, "tonnan": 1}
...
class TestTenhouGameResult:
    ...
    def test_result_creation_custom(self):
        """カスタム結果作成のテスト"""
        result = TenhouGameResult(
            rankings=[2, 1, 4, 3], final_scores=[35000, 30000, 20000, 15000], uma=[10, 5, -5, -10]
        )
    ...
    def test_to_tenhou_format(self):
        """天鳳形式変換のテスト"""
        result = TenhouGameResult(
            rankings=[1, 3, 2, 4], final_scores=[32000, 28000, 25000, 15000], uma=[20, 10, -10, -20]
        )
...
class TestTenhouGameData:
    ...
    def test_game_data_creation(self):
        ...
        game_data = TenhouGameData(
            title="テストゲーム", players=self.players, rule=self.rule, result=self.result
        )
    ...
    def test_game_data_auto_complete_players(self):
        ...
        game_data = TenhouGameData(title="テスト", players=incomplete_players, rule=self.rule)
    ...
    def test_add_action(self):
        """アクション追加のテスト"""
        game_data = TenhouGameData(title="テスト", players=self.players, rule=self.rule)
...
class TestTenhouGameDataBuilder:
    ...
    def test_builder_with_actions(self):
        ...
        game_data = (
            builder.set_title("アクションテスト").add_player("Player1").add_action(action).build()
        )
    ...
    def test_builder_reset(self):
        ...
        # 最初のビルド
        game_data1 = builder.set_title("ゲーム1").add_player("Player1").build()

        # リセット後のビルド
        game_data2 = builder.reset().set_title("ゲーム2").add_player("Player2").build()
    ...
    def test_builder_with_rule_and_result(self):
        ...
        builder = TenhouGameDataBuilder()
        game_data = builder.set_title("完全テスト").set_rule(rule).set_result(result).build()
