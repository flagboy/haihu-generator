...
class TestAdvancedMemoryOptimizer:
    ...
    @pytest.mark.skipif(not hasattr(np, 'get_default_memory_pool'), 
                        reason="NumPy memory pool not available")
    ...
    def test_optimize_torch_memory(self, memory_optimizer):
        ...
        with patch('src.optimization.advanced_memory_optimizer.TORCH_AVAILABLE', False):
            ...
            result = memory_optimizer.optimize_memory()
            torch_results = [r for r in result['optimizations'] 
                           if r.get('operation') == 'torch_optimization']
    ...
    @pytest.mark.skipif(not sys.platform.startswith('linux'), 
                        reason="malloc_trim is Linux-specific")
