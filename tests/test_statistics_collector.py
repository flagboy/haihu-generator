...
class TestStatisticsCollector:
        ...
        return [
            {
                ...
                'classifications': [
                    (Mock(bbox=[10, 10, 50, 50]), Mock(label=f'{i%9+1}m', confidence=0.8 + i*0.01))
                    for _ in range(3)
                ]
            }
            for i in range(10)
    ...
    def test_collect_statistics_with_ai_results(self, statistics_collector, 
                                              sample_processing_result, 
                                              sample_ai_results):
        """AI結果を含む統計収集テスト"""
        stats = statistics_collector.collect_statistics(
            sample_processing_result,
            ai_results=sample_ai_results
        )
    ...
    def test_collect_performance_statistics(self, statistics_collector, sample_processing_result):
        ...
        sample_processing_result.memory_usage = {
            'peak_mb': 512.3,
            'average_mb': 256.7
        }
        sample_processing_result.gpu_usage = {
            'utilization': 85.5,
            'memory_used_mb': 2048
        }
    ...
    def test_format_statistics_report(self, statistics_collector):
        ...
        statistics = {
            ...
            'performance': {
                'frames_per_second': 4.05,
                'tiles_per_second': 10.12
            },
