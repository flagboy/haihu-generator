...
class TestTenhouJsonFormatter:
    ...
    def _create_sample_game_data(self) -> TenhouGameData:
        ...
        rule = TenhouGameRule(
            game_type=TenhouGameType.TONPUU,
            red_dora=True,
            kuitan=True,
            display_name="四麻東風戦"
        )
        ...
        # サンプルアクションを追加
        game_data.add_action(TenhouDrawAction(
            player=0,
            tile=TenhouTile("1m")
        ))
        game_data.add_action(TenhouDiscardAction(
            player=0,
            tile=TenhouTile("9z"),
            is_riichi=False
        ))
    ...
    def test_convert_discard_action(self):
        ...
        # リーチ打牌
        action_riichi = TenhouDiscardAction(
            player=2,
            tile=TenhouTile("7p"),
            is_riichi=True
        )
