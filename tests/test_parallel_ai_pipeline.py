...
from concurrent.futures import TimeoutError, ProcessPoolExecutor, ThreadPoolExecutor
...
class TestParallelAIPipeline:
    ...
    def test_process_frames_parallel_batches_partial_failure(self, parallel_pipeline, sample_frames):
        """並列バッチ処理の部分的失敗テスト"""
        call_count = 0
    ...
    def test_process_batch_wrapper(self, parallel_pipeline):
        ...
        # process_frames_batchのモック
        parallel_pipeline.process_frames_batch = Mock(return_value=PipelineResult(
            success=True,
            frame_results=[],
            total_frames=5,
            total_detections=10,
    ...
    def test_process_frames_with_prefetch(self, parallel_pipeline, sample_frames):
        ...
        def mock_prefetch_and_process(batch, start_frame):
            return PipelineResult(
                success=True,
                frame_results=[
                    {'frame_id': start_frame + i, 'detections': []}
    ...
    def test_timeout_handling(self, parallel_pipeline, sample_frames):
        ...
        def mock_slow_process(frames, start_frame):
            ...
            return PipelineResult(success=True, frame_results=[], total_frames=0,
                                total_detections=0, total_classifications=0,
                                processing_time=100, average_confidence=0,
                                confidence_distribution={})
        ...
        frame_results = [
            {
                'frame_id': i,
                'classifications': [
                    (Mock(), Mock(confidence=0.8 + i * 0.01))
                    for _ in range(3)
                ]
            }
            for i in range(10)
