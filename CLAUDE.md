...
### 重要: 実行前によく考える (Think Carefully Before Execution)
...
- プッシュ後は必ずCIが全て成功することを確認する（`gh pr checks` コマンドを使用）
